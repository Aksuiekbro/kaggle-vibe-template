"""exp_054 (own, probe, scheduler id exp_053): XGBoost hyperparameter sweep on
top of exp_042's winning config (XGBoost + more_oversample rare-class
augmentation). Model-family choice (exp_040), augmentation tuning (exp_037/
038), and their composition (exp_042) are all closed/settled, but XGBoost's
own hyperparameters have used untuned defaults (n_estimators=300, lr=0.05,
max_depth=6, subsample/colsample=0.8) since exp_040 first introduced it.
Distinct axis from exp_010/012's closed LGBM capacity-tuning family (never
tried for XGBoost).

Coordinate search (not full grid, to stay cheap): vary one hyperparameter at
a time away from the exp_042 baseline, augmentation always on. Single 80/20
full-data stratified holdout probe (per exp_012/C13 fidelity lesson), same
split/adversarial-weighting structure as probe_exp043_xgb_augmented.py.
"""
import sys
import time
import warnings
...
BASE = dict(n_estimators=300, learning_rate=0.05, max_depth=6, subsample=0.8, colsample_bytree=0.8)
CONFIGS = [
    ("baseline", BASE),
    ("depth_4", {**BASE, "max_depth": 4}),
    ("depth_8", {**BASE, "max_depth": 8}),
    ("n_est_200", {**BASE, "n_estimators": 200}),
    ("n_est_500", {**BASE, "n_estimators": 500}),
    ("lr_0.03", {**BASE, "learning_rate": 0.03}),
    ("lr_0.08", {**BASE, "learning_rate": 0.08}),
]
...
for name, params in CONFIGS:
    ...
    clf = xgb.XGBClassifier(
        **params, objective="multi:softmax", num_class=n_classes,
        random_state=42, n_jobs=1, tree_method="hist", verbosity=0,
    )
    clf.fit(X_tr_aug, y_tr_aug)
    pred = clf.predict(X_va)
    results[name] = report(name, pred)
    print(f"  ({name} fit+predict done in {time.time()-t1:.0f}s)", flush=True)

print("\n=== exp_054: XGBoost+aug hyperparameter coordinate search, full-data 80/20 holdout ===", flush=True)
base_plain, base_weighted, base_topq = results["baseline"]
for name, _ in CONFIGS:
    plain, weighted, topq = results[name]
    print(f"{name}: delta plain={plain-base_plain:+.4f} weighted={weighted-base_weighted:+.4f} topq={topq-base_topq:+.4f}", flush=True)
