"""exp_061 (own, new mechanism): cross-model AGREEMENT-gated pseudo-labeling,
instead of a single model's own confidence threshold.

Motivated by arxiv 2510.07509 (Efficient Generalization via Multimodal
Co-Training under Data Scarcity and Distribution Shift, logged via
writeup.py): dual-threshold pseudo-labeling + cross-view agreement selects
higher-quality pseudo-labels under distribution shift than a single model's
own confidence. This is a genuinely different mechanism from every prior
pseudo-labeling axis tried this competition:
  - single-model confidence threshold sweep (exp_051/052/055/056/057/058/059)
  - iterative multi-round self-training (exp_055, closed negative)
  - EM label-prior correction (exp_060, closed negative)
and different from blend-of-final-predictions (exp_017/044, closed 2/2
negative) because agreement here only GATES which real test rows get
pseudo-labeled -- the final stage-2 model is XGBoost-alone, never a blend of
XGBoost+LGBM predictions.

Uses the identical harness/split/seed/augmentation config as
exp_051/052/055/056/057/058/059 for direct comparability. Trains XGBoost AND
LGBM stage-1 models on the same augmented train set; a real test row is
pseudo-labeled only if both models agree on the predicted class AND both
confidences clear a threshold. Sweeps threshold against exp_058's winning
single-model config (thresh=0.60, full-CV topq +0.0137, this probe's
XGBOOST_ONLY_REF).
"""
import sys
import time
...
# baseline reference: identical split/seed/features/model as exp_048/049/050/057/058/059
BASELINE_PLAIN = 0.9442
BASELINE_WEIGHTED = 0.8223
BASELINE_TOPQ = 0.9573
THRESH_060_SINGLE_MODEL_TOPQ = 0.9658  # exp_059 probe, single-model conf>=0.60 (scheduler exp_058 full-CV +0.0137)

AGREE_THRESHOLDS = (0.50, 0.60, 0.70)
...
proba_xgb = clf_xgb.predict_proba(X_test_real)
conf_xgb = proba_xgb.max(axis=1)
pred_xgb = proba_xgb.argmax(axis=1)

clf_lgb = make_lgb(n_classes)
clf_lgb.fit(X_tr_aug, y_tr_aug)
proba_lgb = clf_lgb.predict_proba(X_test_real)
conf_lgb = proba_lgb.max(axis=1)
pred_lgb = proba_lgb.argmax(axis=1)

agree_mask_all = pred_xgb == pred_lgb
print(f"\nstage-1 XGBoost+LGBM fit ({time.time()-t0:.0f}s). "
      f"Raw agreement rate on real test: {agree_mask_all.mean():.4f}", flush=True)

print("\n=== exp_061: cross-model agreement-gated pseudo-labeling "
      "(XGBoost+LGBM must agree AND both clear threshold) ===", flush=True)
for thresh in AGREE_THRESHOLDS:
    pl_mask = agree_mask_all & (conf_xgb >= thresh) & (conf_lgb >= thresh)
    n_pl = int(pl_mask.sum())
    if n_pl == 0:
        print(f"\nagree_threshold {thresh}: 0 pseudo-labeled rows, skipping", flush=True)
        continue
    X_pl = X_test_real[pl_mask]
    y_pl = pred_xgb[pl_mask]  # == pred_lgb by construction (agreement-gated)
    X_tr_pl = np.vstack([X_tr_aug, X_pl])
    y_tr_pl = np.concatenate([y_tr_aug, y_pl])
    clf = make_xgb(n_classes)
    clf.fit(X_tr_pl, y_tr_pl)
    pred = clf.predict(X_va)
    print(f"\nagree_threshold {thresh}: {n_pl}/{len(conf_xgb)} real test rows pseudo-labeled "
          f"(agreement-gated), fit done ({time.time()-t0:.0f}s)", flush=True)
    plain, weighted, topq = report(f"agreement_pseudolabel_thresh_{thresh}", pred)
    ...
    print(f"delta topq     vs single-model@0.60:  {topq - THRESH_060_SINGLE_MODEL_TOPQ:+.4f}", flush=True)
