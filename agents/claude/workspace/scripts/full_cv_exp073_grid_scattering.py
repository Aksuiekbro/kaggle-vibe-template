"""exp_073/exp_074 (own, scheduler id exp_072) full 3-fold CV promotion.

Probe (single 80/20 holdout, probe_exp073_scattering.py + deconfounding
control probe_exp074_grid_noaug_control.py) found:
  - scattering-alone: topq=0.6838, catastrophic failure (matches CREPE/PANNs
    pattern -- no non-grid feature family beats grid-harmonic alone).
  - grid+scattering vs grid-only, BOTH unaugmented (scattering features are
    not producible by augment_rows -> extract_from_signal, so augmentation
    is dropped for this comparison; that is why this full CV does NOT
    include a rare-class-augmented variant -- augmentation and scattering
    are not currently composable):
    grid-only-no-aug topq=0.9487 vs grid+scattering-no-aug topq=0.9658,
    deconfounded probe delta +0.0171.

Fold-safe 3-fold CV, same topq gating metric (adversarial-weighted top
quartile) as every other lever this competition. No augmentation in either
arm (apples-to-apples; the augmentation lever is tested/composed separately
and is orthogonal to this feature-family question).
"""
import sys
import time
import warnings
...
# same row-filter for both arms: inner-joined to the scattering set (a few
# files may have failed scattering extraction), so n_train is identical.
...
# adversarial classifier per variant (feature space differs -> different P(is_test))
adv_w = {}
...
print("\n=== exp_073/074 full CV: grid-only vs grid+scattering (both no-aug) ===", flush=True)
results = {}
...
print(f"\ndelta plain:    {plain_s - plain_b:+.4f}", flush=True)
print(f"delta weighted: {weighted_s - weighted_b:+.4f}", flush=True)
print(f"delta topq:     {topq_s - topq_b:+.4f}  <- coordinator's gating metric", flush=True)
print(f"total time: {time.time()-t0:.0f}s", flush=True)

np.save(GRID_DIR + "exp073_pred_grid_only.npy", preds["grid-only"])
np.save(GRID_DIR + "exp073_pred_grid_scattering.npy", preds["grid+scattering"])
np.save(GRID_DIR + "exp073_labels.npy", y)
