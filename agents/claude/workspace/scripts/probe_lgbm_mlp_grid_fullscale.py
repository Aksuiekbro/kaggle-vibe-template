"""exp_017 probe: 2-fold (not 3-fold) StratifiedKFold on FULL 2330-row/82-class
grid features, comparing LGBM-only, MLP-only, and soft-vote blend. Cheaper
signal check before committing to full_cv_lgbm_mlp_blend.py's 3-fold run.

Per exp_010/exp_012 lesson: row-subsampled probes are misleading for this
82-class dataset. This probe cuts fold count (2 instead of 3) on the full
row/class data instead of subsampling rows, matching the fix used in exp_012.
"""
import time
import numpy as np
import pandas as pd
