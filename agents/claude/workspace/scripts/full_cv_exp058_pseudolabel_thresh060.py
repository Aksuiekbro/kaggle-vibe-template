"""exp_058 (scheduler id exp_058, filename exp_059): full 3-fold CV of
fold-honest pseudo-labeling at CONF_THRESHOLD=0.60, the best of the
0.60/0.65/0.70 probe sweep (probe topq +0.0085 vs no-pl, but still below
thresh=0.75's probe +0.0171 and full-CV +0.0069). Confirms/falsifies at
full fidelity whether the probe-scale non-monotonicity (0.75 beats
0.60/0.65/0.70) holds, closing the threshold-sweep axis either way.
...
# --- fold-safe 3-fold CV: XGBoost+aug baseline vs XGBoost+aug+pseudo-label(real test, thresh=0.60) ---
...
print("\n=== exp_058 full CV: XGBoost+aug baseline vs +pseudo-label(real test, thresh=0.60) ===", flush=True)
plain_b, weighted_b, topq_b = report("XGBoost + more_oversample augment (baseline)", correct_base)
plain_p, weighted_p, topq_p = report("XGBoost + more_oversample augment + pseudo-label(0.60)", correct_pl)
...
print(f"delta topq vs exp_056 (thresh=0.75 full CV, +0.0069): compare directly", flush=True)
print(f"total time: {time.time()-t0:.0f}s", flush=True)

np.save(DIR + "exp058_pred_base.npy", pred_base)
np.save(DIR + "exp058_pred_pl.npy", pred_pl)
np.save(DIR + "exp058_labels.npy", y)
