"""exp_072 (scheduler id exp_071, own, new): even-lower confidence-threshold
sweep (0.30/0.40/0.50) for fold-honest pseudo-labeling.

exp_058/059 (scheduler ids exp_057/exp_058) found a probe-vs-full-CV
REVERSAL on this exact axis: at probe scale, thresh=0.75 (topq +0.0171)
ranked above thresh=0.60 (topq +0.0085), but the full 3-fold CV flipped that
-- thresh=0.60's full CV (+0.0137) beat thresh=0.75's full CV (+0.0069) by
~2x. Since the probe-level threshold ranking has already proven unreliable
once on this exact axis, the true optimum may sit below the previously
probed floor (0.60). This sweep fills in the next lower band using the
identical harness/split/seed/augmentation config as exp_051/052/055-059 for
direct comparability. If any config beats thresh=0.60's probe number
(+0.0085), promote to full CV before trusting it (per the established
pattern on this axis).
"""
import sys
import time
import warnings
...
# baseline reference: identical split/seed/features/model as exp_048/049/050/057/059
BASELINE_PLAIN = 0.9442
BASELINE_WEIGHTED = 0.8223
BASELINE_TOPQ = 0.9573
THRESH_075_TOPQ = 0.9744  # exp_057's probe (scheduler id exp_056)
THRESH_060_TOPQ = 0.9658  # exp_059's probe (scheduler id exp_058): BASELINE_TOPQ + 0.0085

CONF_THRESHOLDS = (0.50, 0.40, 0.30)
...
# --- adversarial P(is_test), reused only for topq weighting (same as exp_048/049/050/057/059) ---
...
# --- single 80/20 holdout (same split/seed as exp_048/049/050/057/059) ---
idx = np.arange(len(y))
...
print(f"\nstage-1 model fit ({time.time()-t0:.0f}s). Real test confidence "
      f"percentiles: p10={np.percentile(test_conf, 10):.3f} p25={np.percentile(test_conf, 25):.3f} "
      f"p50={np.percentile(test_conf, 50):.3f} min={test_conf.min():.3f}", flush=True)

print("\n=== exp_072: even-lower confidence-threshold sweep (0.50/0.40/0.30), "
...
for thresh in CONF_THRESHOLDS:
    ...
    print(f"delta topq     vs thresh=0.60: {topq - THRESH_060_TOPQ:+.4f}", flush=True)
