"""exp_057 (own, new): finer confidence-threshold sweep around exp_051/052's
winning thresh=0.85 for fold-honest pseudo-labeling.

exp_051/052 only swept a coarse 3-point grid (0.7/0.85/0.95); 0.85 won but
the true optimum could sit anywhere in between -- this fills in 0.75/0.80/0.90
around it with the identical harness/split/seed/augmentation config used by
exp_051/052/055/056, so results are directly comparable to the banked
thresh=0.85 topq=0.9658 result. Distinct from exp_055 (which changed the
ACCEPTANCE SCORE, confidence+density) and exp_056 (which changed the number
of ROUNDS) -- this is purely finer resolution on the one hyperparameter
(threshold value) of the already-confirmed single-round mechanism.
...
CONF_THRESHOLDS = (0.90, 0.85, 0.80, 0.75)
THRESH_085_TOPQ = 0.9658  # exp_051/052's banked reference at thresh=0.85
...
print("\n=== exp_057: finer confidence-threshold sweep around 0.85, "
...
for thresh in CONF_THRESHOLDS:
    ...
    print(f"delta topq     vs thresh=0.85: {topq - THRESH_085_TOPQ:+.4f}", flush=True)
