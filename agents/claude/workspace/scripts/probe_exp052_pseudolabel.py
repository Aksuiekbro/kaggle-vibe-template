"""exp_052 (own, new): fold-honest pseudo-labeling -- add high-confidence
predictions on the REAL unlabeled test set as extra training rows, retrain,
and measure the effect on held-out accuracy.

Genuinely new mechanism vs everything tried this competition: every prior
use of the test set (adversarial P(is_test) reweighting/CORAL/diagonal
alignment/feature elimination) only read test's FEATURE distribution. This
is the first experiment where test rows themselves become training data.

Fold-honest simulation of the real deployment: train on the 80% split
(+more_oversample augmentation, exp_042's banked-best config), predict on
the actual unlabeled test set, keep only rows above a confidence threshold
as pseudo-labeled additions, retrain on tr+aug+pseudolabel, and score on the
held-out 20% (real labels, never touched by pseudo-labeling or the
threshold choice). This mirrors exactly what a real self-training submission
pipeline would do (fit on all train, pseudo-label real test, refit, predict
test) so a positive holdout delta here would transfer directly.

Reuses the same 80/20 holdout split/seed and more_oversample augmentation
config as exp_048/049/050 for direct comparability to the banked exp_042
baseline numbers.
"""
import sys
import time
...
# baseline reference: identical split/seed/features/model as exp_048/049/050
BASELINE_PLAIN = 0.9442
BASELINE_WEIGHTED = 0.8223
BASELINE_TOPQ = 0.9573

CONF_THRESHOLDS = (0.95, 0.85, 0.70)
...
# --- adversarial P(is_test), reused only for topq weighting (same as exp_048/049/050) ---
...
# --- single 80/20 holdout (same split/seed as exp_048/049/050) ---
idx = np.arange(len(y))
...
print("\n=== exp_052: XGBoost+aug+pseudo-label(real test), sweep confidence threshold ===", flush=True)
