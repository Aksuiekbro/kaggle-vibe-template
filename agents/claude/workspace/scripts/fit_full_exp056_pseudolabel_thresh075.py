"""exp_056 follow-through: fit XGBoost + exp_037/038's more_oversample
rare-class augmentation + fold-honest pseudo-labeling (CONF_THRESHOLD=0.75,
exp_057's finer threshold sweep winner) on the FULL train set and predict
test. Writes a submission CSV for the score gate.

Full 3-fold CV (full_cv_exp056_pseudolabel_thresh075.py) found this beats the
exp_042/sub_025-equivalent augmented-only baseline on the coordinator's
shift-aware topq metric: plain +0.0009, weighted +0.0187, topq +0.0069.
Recorded via scheduler.py record --id exp_056 --stage full --delta 0.0069.
This is the current best full-CV-confirmed lever since sub_025 -- generating
the actual submission (stage-1 model trained on real full train, pseudo-
labels real test at conf>=0.75, stage-2 model retrained on train+pseudo-
labels, predicts final labels for all test rows).
"""
import sys
import time
import warnings
...
out_path = DATA_DIR + "submission_exp056_xgb_augmented_pseudolabel075.csv"
