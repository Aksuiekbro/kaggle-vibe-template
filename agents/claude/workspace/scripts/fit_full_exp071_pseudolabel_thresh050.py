"""exp_071 follow-through: fit XGBoost + exp_037/038's more_oversample
rare-class augmentation + fold-honest pseudo-labeling (CONF_THRESHOLD=0.50,
exp_071's lower-threshold-sweep winner, superseding exp_058/059's thresh=0.60)
on the FULL train set and predict test. Writes a submission CSV for the score
gate.

Full 3-fold CV (full_cv_exp071_lower_threshold_sweep.py) found thresh=0.50
beats the exp_042/sub_025-equivalent augmented-only baseline on the
coordinator's shift-aware topq metric: topq +0.0154, beating the previously
banked thresh=0.60 (sub_026, topq +0.0137) by +0.0017. Recorded via
scheduler.py record --id exp_071 --stage full --delta 0.0154. thresh=0.40 and
thresh=0.30 were both weaker (topq +0.0103) so 0.50 is the new optimum;
threshold-sweep axis is now closed. Identical pipeline to
fit_full_exp058_pseudolabel_thresh060.py except CONF_THRESHOLD.
"""
import sys
import time
import warnings
...
out_path = DATA_DIR + "submission_exp071_xgb_augmented_pseudolabel050.csv"
