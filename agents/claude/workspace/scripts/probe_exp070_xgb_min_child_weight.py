"""exp_069 (scheduler id) / exp_070 (idea label): XGBoost min_child_weight
regularization sweep on top of exp_042's banked XGBoost+more_oversample
config -- same 80/20 holdout harness as exp_048-069.

Motivated by exp_066/069's emerging pattern: LightGBM's restrictive
min_data_in_leaf=20 default may beat a permissive leaf-size floor under this
dataset's severe train/test shift by resisting overfit to thin-class quirks.
XGBoost's analogous knob (min_child_weight) has never been tuned this
competition -- exp_053/054 covered depth/n_estimators/lr only, and the
default (1) is maximally permissive. Tests whether "more restrictive = more
shift-robust" generalizes to the model family actually banked for submission.
"""
import sys
import time
import warnings
...
for mcw in (1, 5, 10, 20, 40):
    tstart = time.time()
    clf = xgb.XGBClassifier(
        n_estimators=300, learning_rate=0.05, max_depth=6, min_child_weight=mcw,
    ...
    plain, weighted, topq = report(f"xgb_min_child_weight={mcw}", pred)
