...
def extract_all_normalized_features_v2(path, data_dir):
    full_path = os.path.join(data_dir, path)
    try:
        sr, wav = wavfile.read(full_path)
    except Exception as e:
        print(f"Error reading {full_path}: {e}")
        return np.zeros(1888, dtype=np.float32)
    ...
    # Chroma CQT (12 bins)
    try:
        chroma = librosa.feature.chroma_cqt(y=y_segment, sr=sr, fmin=librosa.note_to_hz("C1"), n_chroma=12)
    ...
    try:
        ...
        contrast_std = np.zeros(7, dtype=np.float32)
    ...
    try:
        ...
        flatness_mean = np.zeros(1, dtype=np.float32)
        flatness_std = np.zeros(1, dtype=np.float32)
...
if __name__ == "__main__":
    ...
    print("Extracting features v2 for train...")
    t0 = time.time()
    X_train = Parallel(n_jobs=-1)(delayed(extract_all_normalized_features_v2)(p, data_dir) for p in df_train['Path'])
    X_train = np.array(X_train, dtype=np.float32)
    print(f"Train features shape: {X_train.shape} in {time.time() - t0:.2f}s")
    
    print("Extracting features v2 for test...")
    t0 = time.time()
    X_test = Parallel(n_jobs=-1)(delayed(extract_all_normalized_features_v2)(p, data_dir) for p in df_test['Path'])
    X_test = np.array(X_test, dtype=np.float32)
    print(f"Test features shape: {X_test.shape} in {time.time() - t0:.2f}s")
    
    np.save(os.path.join(cache_dir, "X_train_norm_v2.npy"), X_train)
    np.save(os.path.join(cache_dir, "X_test_norm_v2.npy"), X_test)
    print("Features v2 successfully saved to disk.")
