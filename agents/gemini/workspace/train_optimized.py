...
if os.path.exists(X_train_path) and os.path.exists(X_test_path) and os.path.exists(y_train_path):
    ...
    t0 = time.time()
    X_train = Parallel(n_jobs=-1)(delayed(extract_features_fast)(p) for p in df_train['Path'])
    ...
    t0 = time.time()
    X_test = Parallel(n_jobs=-1)(delayed(extract_features_fast)(p) for p in df_test['Path'])
...
sub_path = "agents/gemini/submissions/submission_lgb_opt.csv"
