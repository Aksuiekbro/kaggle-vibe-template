...
# Define LightGBM parameters (tuned for grid-only speed)
lgb_params = {
...
oof_pca_mlp = np.zeros((len(X_train), 82))
test_pca_mlp = np.zeros((len(X_test), 82))
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
    ...
    oof_pca_mlp[val_idx] = fold_val
    test_pca_mlp += fold_te / 5.0
    acc = accuracy_score(y_va, np.argmax(fold_val, axis=1))
    print(f"Fold {fold} PCA-MLP Accuracy: {acc:.5f} in {time.time() - t_start:.2f}s")
    ...
    print(f"Fold {fold} completed in {time.time() - t0:.2f}s")
...
for name, oof in models.items():
    ...
    print(f"{name:<10} OOF Accuracy: {acc:.6f}")
...
for name, weight in zip(model_names, best_weights):
    print(f"Weight for {name:<10}: {weight:.4f}")
...
np.save(os.path.join(cache_dir, "oof_probs_mlp_grid_only.npy"), oof_pca_mlp)
np.save(os.path.join(cache_dir, "test_probs_mlp_grid_only.npy"), test_pca_mlp)
np.save(os.path.join(cache_dir, "oof_probs_svm_grid_only.npy"), oof_svm_pca)
np.save(os.path.join(cache_dir, "test_probs_svm_grid_only.npy"), test_svm_pca)
np.save(os.path.join(cache_dir, "oof_probs_lgb_grid_only.npy"), oof_lgb)
np.save(os.path.join(cache_dir, "test_probs_lgb_grid_only.npy"), test_lgb)

# Save final submission
os.makedirs("agents/gemini/submissions", exist_ok=True)
sub_path = "agents/gemini/submissions/submission_grid_only.csv"
...
print(f"\nSaved grid-only ensemble submission to {sub_path}")
...
results_path = os.path.join(cache_dir, "results_grid_only.json")
