...
def objective(trial):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
        ...
        # We can pass early_stopping_rounds to the fit call
        model = xgb.XGBClassifier(**params, early_stopping_rounds=15)
        model.fit(
            X_tr, y_tr,
            eval_set=[(X_va, y_va)],
            verbose=False
...
print("Starting Optuna hyperparameter search on combined clean features for XGBoost (12 trials)...", flush=True)
t0 = time.time()
...
print("\nTraining final XGBoost model with best parameters...", flush=True)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
    ...
    model = xgb.XGBClassifier(**final_params, early_stopping_rounds=15)
    model.fit(
        X_tr, y_tr,
        eval_set=[(X_va, y_va)],
        verbose=False
...
# Save outputs
np.save(os.path.join(cache_dir, "oof_probs_clean_xgb.npy"), oof_probs)
np.save(os.path.join(cache_dir, "test_probs_clean_xgb.npy"), test_probs)
...
with open(os.path.join(cache_dir, "results_clean_xgb.json"), "w") as f:
