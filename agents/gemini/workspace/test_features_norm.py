...
t0 = time.time()
feats = []
for p in df_train['Path'][:5]:
    t_start = time.time()
    f = extract_all_normalized_features(p, data_dir)
    print(f"Extracted features for {p} shape: {f.shape} in {time.time() - t_start:.3f}s")
    feats.append(f)
feats = np.array(feats)
print(f"Total time for 5 samples: {time.time() - t0:.3f}s, shape: {feats.shape}")
