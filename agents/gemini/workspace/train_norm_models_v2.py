...
X_train = np.load(os.path.join(cache_dir, "X_train_norm_v2.npy"))
X_test = np.load(os.path.join(cache_dir, "X_test_norm_v2.npy"))
y_train = np.load(os.path.join(cache_dir, "y_train.npy"))

print(f"Loaded normalized features v2 shape: Train {X_train.shape}, Test {X_test.shape}")
...
print("\n--- OOF Accuracies ---")
oof_acc_mlp_pca = accuracy_score(y_train, np.argmax(oof_mlp_pca, axis=1))
oof_acc_svm_pca = accuracy_score(y_train, np.argmax(oof_svm_pca, axis=1))
oof_acc_mlp_raw = accuracy_score(y_train, np.argmax(oof_mlp_raw, axis=1))
oof_acc_lgb = accuracy_score(y_train, np.argmax(oof_lgb, axis=1))

print(f"PCA-MLP (5 seeds) OOF Accuracy: {oof_acc_mlp_pca:.6f}")
print(f"PCA-SVM OOF Accuracy: {oof_acc_svm_pca:.6f}")
print(f"Raw MLP (5 seeds) OOF Accuracy: {oof_acc_mlp_raw:.6f}")
print(f"LightGBM OOF Accuracy: {oof_acc_lgb:.6f}")

# Save model OOF/test probabilities
np.save(os.path.join(cache_dir, "oof_probs_mlp_pca_v2.npy"), oof_mlp_pca)
np.save(os.path.join(cache_dir, "test_probs_mlp_pca_v2.npy"), test_mlp_pca)

np.save(os.path.join(cache_dir, "oof_probs_svm_pca_v2.npy"), oof_svm_pca)
np.save(os.path.join(cache_dir, "test_probs_svm_pca_v2.npy"), test_svm_pca)

np.save(os.path.join(cache_dir, "oof_probs_mlp_raw_v2.npy"), oof_mlp_raw)
np.save(os.path.join(cache_dir, "test_probs_mlp_raw_v2.npy"), test_mlp_raw)

np.save(os.path.join(cache_dir, "oof_probs_lgb_v2.npy"), oof_lgb)
np.save(os.path.join(cache_dir, "test_probs_lgb_v2.npy"), test_lgb)

# Optimize blend weights
models = {
    "mlp_pca_v2": oof_mlp_pca,
    "svm_pca_v2": oof_svm_pca,
    "mlp_raw_v2": oof_mlp_raw,
    "lgb_v2": oof_lgb
}

test_probs_dict = {
    "mlp_pca_v2": test_mlp_pca,
    "svm_pca_v2": test_svm_pca,
    "mlp_raw_v2": test_mlp_raw,
    "lgb_v2": test_lgb
}
...
sub_path = "agents/gemini/submissions/submission_norm_blend_v2.csv"
...
results_path = os.path.join(cache_dir, "results_norm_blend_v2.json")
